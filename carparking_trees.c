...
TempBPTreeNode* findParentTempNode(TempBPTreeNode* current, TempBPTreeNode* child);
void insertInternalTempNode(int key, TempBPTreeNode* parent, TempBPTreeNode* child, TempBPTreeNode** root);
void insertInternalTempNodeDescending(int key, TempBPTreeNode* parent, TempBPTreeNode* child, TempBPTreeNode** root);
void freeTempTree(TempBPTreeNode* root);
void traverseTempTreeForOccupancy(TempBPTreeNode* root);
void traverseTempTreeForRevenue(TempBPTreeNode* root);
void traverseTempTreeForDisplay(TempBPTreeNode* root);
...
void arrangeParkingSpacesByOccupancy() {
    if (parking_space_tree == NULL) {
        printf("No parking spaces found.\n");
        return;
    }
    ...
    while (current != NULL) {
        for (int i = 0; i < current->numKeys; i++) {
            insertIntoTempTreeDescending(&tempTree, current->spaces[i], current->spaces[i]->occupancy_count);
        }
        current = current->next;
    }

    printf("\nParking Spaces Sorted by Occupancy (Highest to Lowest):\n");
    printf("--------------------------------------\n");
    printf("| %-10s | %-12s |\n", "Space ID", "Occupancy");
    printf("--------------------------------------\n");

    if (tempTree != NULL) {
        traverseTempTreeForOccupancy(tempTree);
    }
    printf("--------------------------------------\n");

    freeTempTree(tempTree);
}
...
void arrangeParkingSpacesByRevenue() {
    ...
    while (current != NULL) {
        for (int i = 0; i < current->numKeys; i++) {
            insertIntoTempTreeDescending(&tempTree, current->spaces[i], current->spaces[i]->space_revenue);
        }
        current = current->next;
    }

    printf("\nParking Spaces Sorted by Revenue (Highest to Lowest):\n");
    printf("--------------------------------------\n");
    printf("| %-10s | %-10s |\n", "Space ID", "Revenue");
    printf("--------------------------------------\n");

    if (tempTree != NULL) {
        traverseTempTreeForRevenue(tempTree);
    }
    printf("--------------------------------------\n");
...
void insertInternalParkingSpace(int key, ParkingSpaceNode* parent, ParkingSpaceNode* child) {
    if (parent->numKeys < MAX_KEYS) {
        ...
        while (i >= 0 && key < parent->keys[i]) {
            ...
            parent->spaces[i + 2] = parent->spaces[i + 1];
            i--;
        }
        parent->keys[i + 1] = key;
        parent->spaces[i + 2] = child;
        parent->numKeys++;
    } else {
        int tempKeys[MAX_KEYS + 1];
...
void displayParkingSpaces(ParkingSpaceNode* root) {
    ...
    while (current != NULL) {
        for (int i = 0; i < current->numKeys; i++) {
            insertIntoTempTreeDescending(&tempTree, current->spaces[i], current->spaces[i]->parking_space_ID);
        }
        current = current->next;
    }

    printf("\nParking Spaces (Sorted by ID - Highest to Lowest):\n");
    printf("---------------------------------------------------\n");
    printf("| %-10s | %-10s | %-10s | %-10s |\n", "Space ID", "Status", "Occupancy", "Revenue");
    printf("---------------------------------------------------\n");

    if (tempTree != NULL) {
        traverseTempTreeForDisplay(tempTree);
    }
    printf("---------------------------------------------------\n");
...
// Insert into TempBPTreeNode in descending order
void insertIntoTempTree(TempBPTreeNode** root, ParkingSpace* space, int key) {
    if (*root == NULL) {
    ...
    // Navigate to the appropriate leaf node (descending order)
    while (!current->isLeaf) {
        parent = current;
        int i = 0;
    ...
    // Insert into leaf node (descending order)
    if (current->numKeys < MAX_KEYS) {
        int i = current->numKeys - 1;
        ...
        if (current == *root) {
            ...
            *root = newRoot;
            ...
            insertInternalTempNode(newLeaf->keys[0], findParentTempNode(*root, current), newLeaf, root);
        }
    }
}
...
// Helper function to find parent of a TempBPTreeNode
TempBPTreeNode* findParentTempNode(TempBPTreeNode* current, TempBPTreeNode* child) {
    if (current == NULL || current->isLeaf) return NULL;

    for (int i = 0; i <= current->numKeys; i++) {
        if (current->children[i] == child) return current;
        TempBPTreeNode* p = findParentTempNode(current->children[i], child);
        if (p != NULL) return p;
    }
    return NULL;
}

// Helper function to insert into internal nodes of TempBPTree
void insertInternalTempNode(int key, TempBPTreeNode* parent, TempBPTreeNode* child, TempBPTreeNode** root) {
    if (parent->numKeys < MAX_KEYS) {
        int i = parent->numKeys - 1;
        while (i >= 0 && key < parent->keys[i]) {
        ...
        // Handle internal node splitting for descending order
        int tempKeys[MAX_KEYS + 1];
        TempBPTreeNode* tempChildren[MAX_KEYS + 2];
        ...
        // Insert new key maintaining descending order
        int i = MAX_KEYS - 1;
        while (i >= 0 && key > tempKeys[i]) {
            tempKeys[i + 1] = tempKeys[i];
            tempChildren[i + 2] = tempChildren[i + 1];
        ...
        // Split internal node
        TempBPTreeNode* newInternal = createTempNode(false);
        parent->numKeys = MIN_KEYS;
        newInternal->numKeys = MAX_KEYS - MIN_KEYS;
        ...
        // Handle root creation or propagate split
        if (parent == *root) {
            TempBPTreeNode* newRoot = createTempNode(false);
            newRoot->keys[0] = tempKeys[MIN_KEYS];
            newRoot->children[0] = parent;
            ...
            *root = newRoot;
            ...
            insertInternalTempNodeDescending(tempKeys[MIN_KEYS], findParentTempNode(*root, parent), newInternal, root);
        }
    }
}

// Helper function to insert into internal nodes of TempBPTree (descending order)
void insertIntoTempTreeDescending(TempBPTreeNode** root, ParkingSpace* space, int key) {
    if (*root == NULL) {
        *root = createTempNode(true);
        (*root)->keys[0] = key;
        (*root)->spaces[0] = space;
    ...
    if (current->numKeys < MAX_KEYS) {
        ...
        if (current == *root) {
            ...
            *root = newRoot;
            ...
            insertInternalTempNodeDescending(newLeaf->keys[0], findParentTempNode(*root, current), newLeaf, root);
        }
    }
}


// Function to free the temporary B+ tree
void freeTempTree(TempBPTreeNode* root) {
    if (root == NULL) return;
    if (!root->isLeaf) {
        for (int i = 0; i <= root->numKeys; i++) {
            freeTempTree(root->children[i]);
        }
    }
    free(root);
}
